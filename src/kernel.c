...
void kernel_main() {
    ...
    print_string("Commands: (c) clear, (h) help, (v) version, (t) time, (p) peek, (m) mem, (u) uptime, (s) screen, (b) beep, (q) quiet, (x) exit, (r) reboot, (i) info, (w) who", 0x07, 0, 2);
                ...
                if (c == '\n') {
                    ...
                    cursor_y++;
                        if (command_buffer[0] == 'c' && command_buffer[1] == '\0') {
                            ...
                            print_string("HELP: c=clear, h=help, v=version, t=time, p=peek, m=mem, u=uptime, s=screen, b=beep, q=quiet, x=exit, r=reboot, i=info, w=who.", 0x0E, 0, cursor_y++);
                    } else if (command_buffer[0] == 'v' && command_buffer[1] == '\0') {
                        print_string(message, 0x0B, 0, cursor_y++);
                    } else if (command_buffer[0] == 'w' && command_buffer[1] == '\0') {
                        print_string("WHO: Root User (Clawdbot Agent)", 0x0F, 0, cursor_y++);
